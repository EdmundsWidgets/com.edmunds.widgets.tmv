...
function getAdditionalStyleUrl(widget, options) {
    if (widget === 'nvc') {
        ...
        var size = options.size || 'largeButton';
        return '/ppb/css/main' + '-' + options.color + '-' + size + '.css';
    }
}

function map(baseUrl,array) {
    for (var i = 0; i < array.length; i++) {
        array[i] = baseUrl + array[i];
    }
    return array;
}
...
EDM.createWidget = function(config) {
    ...
    var iframe = new WidgetFrame(config.renderTo, {
            ...
            scripts: map(baseUrl,config.scripts)
        }),
